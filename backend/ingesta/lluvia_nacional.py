...
Cajamarca, 12 que la ingesta no tiene porque son hidrológicas y map_red_graf.php no les da
lluvia), con el umbral de referencia de SENAMHI de cada una. Ver
backend/connectors/senamhi_umbrales.py.
...
Resumen del latido 'lluvia_nacional':
  ...
  sobre_umbral    vigentes cuya lluvia de la hora superó el umbral de referencia
                  (pp_1h > umbral_1h, el mismo criterio del mapa)
  ...
  avisos          detalle: fuentes caídas, puntos descartados, capa desactualizada...
...
def actualizar(ahora: datetime | None = None) -> dict:
    ...
    # 2) Escritura y latido en una sola transacción.
